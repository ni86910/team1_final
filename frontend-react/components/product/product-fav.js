...
function ProductFav({ favInfo, setToggleBtn, toggleBtn }) {
  ...
  // 刪除
  const removeFav = () => {
    const url = `${API_SERVER}/product/remove-fav`
    try {
      fetch(url, {
        method: 'DELETE',
        headers: {
  ...
  // 新增
  const addFav = () => {
    const url = `${API_SERVER}/product/add-fav`
    try {
      fetch(url, {
        method: 'POST',
        headers: {
